(function ($) {
  ...
  $(function () {
    function initStickySidebar() {
      ...
      var $header = $(".gc-js-header-wrapper").not(
        ".gc-header__wrapper--static"
      );
  ...
  $(window).on("load", function () {
    ...
    function adaptBodyGap() {
      ...
      if (isFixed) {
        $(body).css(
          "padding-top",
          headerWrapperHeight + wpadminBarHeight + "px"
        );
      } else {
        $(aboveHeaderBar).css({
          position: "fixed",
  ...
  // Mobile menu
  $(".gc-header__sidebar-menu .menu-item-has-children").on(
    "click",
    function (e) {
  ...
  $(function () {
    ...
    var macyColumnsNumber =
      typeof blogGridColumns !== typeof undefined ? blogGridColumns : 2;
    ...
    if (width > 980) {
      ...
      // Archive grid.
      var $gridCategory = document.querySelector(
        ".archive.date .content--grid"
      );
      if ($gridCategory) {
  ...
  $(function () {
    ...
    var registerLinkWrapper = $(
      ".landing .ps-page--register-main .ps-form__row--submit"
    );
    var landingRowLogin = $(".landing__grid .landing__row:last-child");

    if (
      $(landingRegisterForm).is(":visible") ||
      $(landingRegisterForm).is(":hidden")
    ) {
      ...
      $(this)
        ...
        .each(function () {
          ...
          var subMenu_width = $(this).find(".sub-menu").width(),
            total_subMenu_width =
              parseInt(subMenu_width) * parseInt(levelLength),
          ...
          if (!$(body).hasClass("rtl")) {
            ...
            if (subMenu_totalWidth > headerWidth) {
              $(this)
                .parents(".menu-item")
                .addClass("gc-header__menu-item--reverse");
            }
          }
        });
    });
  ...
  $(".gc-header__menu .woo-wallet-menu-contents")
    .parent()
    .addClass("gc-header__menu-item");
  ...
  $(function () {
    ...
    if ($(".main").hasClass("main--left")) {
      $(".main")
        .parents("body")
        .find(".gc-navbar--sticky")
        .addClass("gc-navbar--right");
    } else if ($(".main").hasClass("main--right")) {
      $(".main")
        .parents("body")
        .find(".gc-navbar--sticky")
        .addClass("gc-navbar--left");
    }
  });
  ...
  function check_cart_if_empty() {
    ...
    if (
      $(wocart_wrapper_empty).find(".hide_cart_widget_if_empty").length !== 0
    ) {
      $(wocart_wrapper).hide();
    } else {
      $(wocart_wrapper).show();
